...
test("spellItemData overwrites a method the source document already carried", () => {
	const src = {
		toObject: () => ({ _id: "x", name: "Hex", type: "spell", system: { level: 1, method: "", prepared: 0 } }),
	};
...
test("spellItemData sets per-day uses for innate casting", () => {
	...
	expect(d2.system.uses).toEqual({ max: "2", spent: 0, recovery: [{ period: "day", type: "recoverAll" }] });
});
...
function harness({ locked = true, failOn = null } = {}) {
	...
	const page = {
		uuid: "page1",
		system: { spells: ["Compendium.x.Item.old"] },
		updates: [],
		async update(d) {
			this.updates.push(d);
		},
	...
	const pack = {
		collection: `${MODULE_ID}.monsters`,
		locked,
		configured: [],
		async configure(c) {
			this.configured.push(c);
			this.locked = c.locked ?? this.locked;
		},
		async getDocument() {
			return actor;
		},
...
test("aborts without writing when a pack cannot be unlocked", async () => {
	...
	await expect(applyPlan({ writes: [MONSTER_WRITE] }, { deps: h.deps })).rejects.toThrow(
...
test("appends to a spell list without dropping what is already there", async () => {
	...
	expect(h.page.updates[0]["system.spells"].sort()).toEqual(["Compendium.x.Item.2", "Compendium.x.Item.old"]);
});
		...
		spells: [
			{
				...
				prep: "prepared",
				perDay: null,
				match: { uuid: "Compendium.x.Item.1" },
			},
		],
	};
...
test("collectState reads existing spell names off the actor", async () => {
	const manifest = {
		monsters: { m1: { name: "Fel Imp", pack: "monsters", spells: [] } },
		spellLists: {},
		aliases: {},
	};
	const packActor = {
	...
	const s = await collectState({ manifest, targets: [TARGETS.MONSTERS], destination: DESTINATIONS.COMPENDIUM, deps });
...
test("collectState finds world actors imported from our pack", async () => {
	...
	const s = await collectState({ manifest, targets: [TARGETS.MONSTERS], destination: DESTINATIONS.WORLD, deps });
