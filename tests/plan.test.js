...
function state({ have = {}, listHave = [], listHaveKeys = [], scopes = { m1: "pack", m2: "pack" } } = {}) {
	return {
		monsters: Object.keys(MANIFEST.monsters).map((id) => ({
			id,
			name: MANIFEST.monsters[id].name,
			scope: scopes[id],
...
test("a fully satisfied plan is empty — the second run adds nothing", () => {
	const p = buildPlan({
		...
		targets: ALL,
		destination: DESTINATIONS.BOTH,
		state: state({ have: { m1: ["ice knife"], m2: ["hex"] }, listHave: ["Compendium.x.Item.1"] }),
	});
...
test("compendium destination skips world-scoped monsters", () => {
	...
	expect(p.writes.filter((w) => w.kind === "monster").map((w) => w.targetName)).toEqual(["Fel Imp"]);
});
...
test("world destination skips pack-scoped monsters", () => {
	...
	expect(p.writes.filter((w) => w.kind === "monster").map((w) => w.targetName)).toEqual(["Frost Revenant"]);
});

test("a monster in the manifest but absent from state is ignored", () => {
	const s = state();
	s.monsters = s.monsters.filter((m) => m.id === "m1");
	const p = buildPlan({ manifest: MANIFEST, index, targets: ALL, destination: DESTINATIONS.BOTH, state: s });
	expect(p.writes.filter((w) => w.kind === "monster").length).toBe(1);
});

test("the same monster imported twice gets one write each", () => {
	const s = state();
	s.monsters.push({ id: "m1", name: "Frost Revenant", scope: "world", uuid: "Actor.copy", haveKeys: new Set() });
	const p = buildPlan({ manifest: MANIFEST, index, targets: ALL, destination: DESTINATIONS.BOTH, state: s });
...
test("an empty index puts everything in not-found and plans nothing", () => {
	const p = buildPlan({
		manifest: MANIFEST,
		index: { get: () => undefined, size: 0, failed: [] },
		targets: ALL,
		destination: DESTINATIONS.BOTH,
		state: state(),
	});
...
test("a spell already on a list is matched by name, not just by uuid", () => {
	...
	const p = buildPlan({
		...
		targets: ALL,
		destination: DESTINATIONS.BOTH,
		state: state({ listHave: ["Compendium.OLD.Item.99"], listHaveKeys: ["ice knife"] }),
	});
	expect(p.counts.listEntries).toBe(0);
		...
		monsters: [
			{
				...
				name: "Doubled Ghoul",
				scope: "pack",
				uuid: "Compendium.wc5e-foundryvtt.monsters.Actor.m3",
				haveKeys: new Set(),
			},
		],
		lists: [],
