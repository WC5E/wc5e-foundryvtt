...
test("applies aliases last", () => {
	expect(normaliseName("Call Lighting", { "call lightning": "x", "call lighting": "call lightning" })).toBe(
		"call lightning",
	);
});
...
test("matches the keys Python wrote, for every record in the real manifest", () => {
	...
	for (const r of records) {
		expect(
			normaliseName(r.name, m.aliases),
...
test("manifestTotals counts documents and references separately", () => {
	...
	expect(manifestTotals(m)).toEqual({ monsters: 2, monsterSpells: 3, lists: 1, listSpells: 3 });
});
