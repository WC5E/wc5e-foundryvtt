...
export const reportConversion = (outputDir: string, result: ConversionResult): void => {
	console.log(
		`Wrote ${result.actors.length} actor files to ${outputDir} from wc5e-mom-full.json in ${Object.keys(result.folders).length} folders`,
	);
	...
	if (unmatchedCounts.length) {
		const top = unmatchedCounts
			.slice(0, 20)
			.map(([name, spellCount]) => `${name}(x${spellCount})`)
			.join(", ");
	...
	if (result.droppedSpellFragments.length) {
		console.log(
			`  ${result.droppedSpellFragments.length} mis-split statblock fragment(s) dropped (a pact-magic header HEADER cannot parse; the spells inside them are not embedded and not in the manifest):`,
		);
...
export const updateMonsterManifest = (result: ConversionResult): void => {
	...
	console.log(
		`  auto-assign manifest: ${Object.keys(records).length} monsters, ${manifestSpellCount} spell references`,
	);
};
