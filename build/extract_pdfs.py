...
of the repo. What gets committed is the extracted text under reference/pdf-extracts/,
...
OUT = os.path.join(REPO, "reference", "pdf-extracts")
...
def main():
    if not shutil.which("pdftotext"):
        sys.exit("  pdftotext not found -- install poppler-utils, or skip this step:\n"
                 "  reference/pdf-extracts/ is committed, so a normal build doesn't need it.")
    src = find_dir()
    if not src:
        sys.exit("  no PDF directory found. Set WC5E_PDF_DIR, or place the class PDFs in\n"
                 "  ../wc5e-class-pdfs/. reference/pdf-extracts/ is committed, so this is optional.")
