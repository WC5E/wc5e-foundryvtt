...
export class AutoAssignApp extends HandlebarsApplicationMixin(ApplicationV2) {
	static DEFAULT_OPTIONS = {
		...
		window: { title: "WC5E.AutoAssign.Title", icon: "fa-solid fa-wand-magic-sparkles", resizable: true },
	...
	static PARTS = {
		...
		body: {
			template: "modules/wc5e-foundryvtt/templates/auto-assign/configure.hbs",
		...
		if (options.isFirstRender) {
			try {
				...
				ui.notifications.error(
					game.i18n.format("WC5E.AutoAssign.LoadFailed", { error: err.message ?? String(err) }),
				);
			...
			buttons: [
				{
					...
					label: "WC5E.AutoAssign.Scan",
					disabled: this.#busy || !this.#checked.size,
				},
			],
		};
	}
			...
			writes: this.#plan.writes.map((w) => ({
				...
				scopeLabel:
					w.kind === "list"
						...
						: game.i18n.localize(
								w.scope === "pack" ? "WC5E.AutoAssign.ScopePack" : "WC5E.AutoAssign.ScopeWorld",
							),
			})),
			buttons: [
				{ type: "button", action: "back", icon: "fa-solid fa-arrow-left", label: "WC5E.AutoAssign.Back" },
				{
					type: "button",
					action: "apply",
	...
	#notFoundContext() {
		...
		for (const n of this.#plan.notFound) {
			...
			groups.get(key).push({
				...n,
				wantedByLabel:
					n.wantedBy.slice(0, 3).join(", ") + (n.wantedBy.length > 3 ? ` +${n.wantedBy.length - 3}` : ""),
			});
		}
	...
	#reportContext() {
		...
		return {
			...
			applied: this.#plan.writes
				.filter((w) => !failed.has(w.targetName))
				.map((w) => ({
			...
			buttons: [
				{ type: "button", action: "back", icon: "fa-solid fa-arrow-left", label: "WC5E.AutoAssign.Done" },
			],
		};
	}
	...
	_onRender(context, options) {
		...
		for (const [name, key] of [
			["target-monsters", "monsters"],
			["target-lists", "spellLists"],
		]) {
			this.element.querySelector(`[name="${name}"]`)?.addEventListener("change", async (ev) => {
				await game.settings.set(MODULE_ID, SETTINGS.targets, { ...this.#targets, [key]: ev.target.checked });
				this.render();
			});
		}
	}
	...
	async #onToggleNode(ev) {
		...
		await game.settings.set(MODULE_ID, SETTINGS.packs, selectedPackIds(this.#tree, this.#checked));
		this.render();
	}
	...
	static async #onScan() {
		...
		try {
			...
			const index = await buildSearchIndex(packIds, {
				aliases: this.#manifest.aliases,
				onProgress: progress,
			});
			this.#indexFailures = index.failed;
			progress(packIds.length, packIds.length, game.i18n.localize("WC5E.AutoAssign.ProgressReading"));
			const state = await collectState({ manifest: this.#manifest, targets, destination: this.#destination });
			this.#plan = buildPlan({ manifest: this.#manifest, index, targets, destination: this.#destination, state });
			this.#stage = "preview";
		} catch (err) {
	...
	static async #onListSpells() {
		const names = [...new Set(this.#plan.notFound.map((n) => n.name))].sort((a, b) => a.localeCompare(b));
	...
	static async #textDialog(titleKey, text) {
		...
		return foundry.applications.api.DialogV2.wait({
			...
			content: `<textarea class="wc5e-aa-textdump" readonly rows="24">${foundry.utils.escapeHTML(
				text || empty,
			)}</textarea>`,
			buttons: [
				{
	...
	#progressReporter(labelKey) {
		...
		return (done, total, what) => {
			...
			if (label) {
				label.textContent = game.i18n.format(labelKey, { done, total, what: what ?? "" });
			}
		};
	}
}
